...
class PlanningGraph():
    ...
    def add_action_level(self, level):
        ...
        self.a_levels.append(set())     # set of action nodes - empty to start
        for action in self.all_actions: # for each action
            node_a = PgNode_a(action)   # define action node

            for node_s in self.s_levels[level]: 
            # for each state node in layer s_levels(level)
                if set(node_a.prenodes).issubset(self.s_levels[level]): # check that node_a prenodes are in s_levels[level]
                    self.a_levels[level].add(node_a) # add action node to action level

                if node_s in node_a.prenodes:   # if node_s is one of the prenodes of node_a:
                    node_s.children.add(node_a) # node_a is a child of node_s
                    node_a.parents.add(node_s)  # node_s as a parent of node_a 
    ...
    def add_literal_level(self, level):
        ...
        self.s_levels.append(set())              # set of literal nodes - empty to start

        for node_a in self.a_levels[level-1]:    # for each action node in previous action level
            for node_s in node_a.effnodes:       # for each literal node in the action node effects
                self.s_levels[level].add(node_s) # add literal node to literal level
                node_s.parents.add(node_a)       # node_a is a parent of node_s
                node_a.children.add(node_s)      # node_s is a child of node_a
    ...
    def inconsistent_effects_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        # given a pair of actions, one of these actions negates an effect of the  
        # other if the actions are in opposite add/rem lists in different nodes
    ...
    def interference_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        for effect in node_a1.action.effect_add:      # node_a1 effects
            if effect in node_a2.action.precond_neg:  # node_a2 preconditions
                return True
        for effect in node_a1.action.effect_rem:
            if effect in node_a2.action.precond_pos:
                return True

        for effect in node_a2.action.effect_add:      # node_a2 effects
            if effect in node_a1.action.precond_neg:  # node_a1 preconditions
                return True
    ...
    def competing_needs_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        return any(p2 in p1.mutex for p1 in node_a1.parents for p2 in node_a2.parents)
    ...
    def inconsistent_support_mutex(self, node_s1: PgNode_s, node_s2: PgNode_s):
        ...
        return not any(set(node_s2.parents).difference(set(a1.mutex)) for a1 in node_s1.parents)
    ...
    def h_levelsum(self) -> int:
        ...
        for goal in self.problem.goal:
            level_sum += self.get_level(goal)
        return level_sum


    def get_level(self, goal) -> int:
        """Given a goal, this method returns the level at which the goal first appears 
        in the planning graph.

        :return: int
        """
        for i in range(len(self.s_levels)): # for each possible level (starting from 0)
            for s in self.s_levels[i]:      # check if there is an S-node having goal as pos literal
